...
class PopSpikeEncoderRegularSpike(nn.Module):
    ...
    def forward(self, obs, batch_size=256):
        """
...
class SpikeMLP(nn.Module):
    ...
    def forward(self, in_pop_spikes, batch_size=256):
        """
...
class SquashedGaussianPopSpikeActor(nn.Module):
    ...
    def forward(self, obs, batch_size=256, deterministic=False, with_logprob=True):
        """
        :param obs: observation
        :param batch_size: batch size
        ...
        # print("out_pop_activity",out_pop_activity)
        # print("out_pop_activity size",out_pop_activity.size())
        mu = self.decoder(out_pop_activity)
        log_std = self.log_std_network(obs)
        log_std = torch.clamp(log_std, LOG_STD_MIN, LOG_STD_MAX)
        var = torch.exp(2 * log_std)
        base_distribution = distributions.Independent(
            distributions.Normal(loc=mu, scale=torch.sqrt(var)), 1
        )
