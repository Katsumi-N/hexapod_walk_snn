...
def main():
    ...
    def make_env(process_idx, test):
        #env = gym.make('dm2gym:HexapodEscape-v0')
    ...
    #sample_env = gym.make('dm2gym:HexapodEscape-v0')
    sample_env = gym.make('dm2gym:HexapodWalk-v0')
    timestep_limit = sample_env.spec.max_episode_steps
    from gym.spaces.utils import flatten_space
    obs_space = flatten_space(sample_env.observation_space)
    action_space = sample_env.action_space
    # print("obs", sample_env.observation_space)
...
# policyをSNNに変更
    import math
    policy = SquashedGaussianPopSpikeActor(obs_space.shape[0], action_space.shape[0], 10, 10, (256, 256), mean_range=(-3,3), std=math.sqrt(0.15),spike_ts=5, act_limit=action_space.high[0],device='cuda')

    import itertools
    policy_params = itertools.chain(policy.encoder.parameters(),policy.snn.parameters(), policy.decoder.parameters(), policy.log_std_network.parameters())
    policy_optimizer = torch.optim.Adam(policy_params, lr=3e-4)
    ...
    for i in policy.parameters():
        if i.requires_grad:
            policy_params += i.numel()
    for j in q_func1.parameters():
        if j.requires_grad:
            q_func1_params += j.numel()
    for k in q_func2.parameters():
        if k.requires_grad:
            q_func2_params += k.numel()

    print("policy_params",policy_params)
    print("q_func1_params",q_func2_params)
    print("q_func2_params", q_func2_params)
