...
def main():
    ...
    import math
    policy = PopSpikeActor(obs_space.shape[0], action_space.shape[0], 10, 10, (256, 256), mean_range=(-3,3), std=math.sqrt(0.15),spike_ts=5, act_limit=action_space.high[0],device='cuda')
    ...
    import itertools
    policy_params = itertools.chain(policy.encoder.parameters(),policy.snn.parameters(), policy.decoder.parameters())
    policy_optimizer = torch.optim.Adam(policy_params, lr=5e-4)
