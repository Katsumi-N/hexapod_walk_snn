...
# sys.path.append('../../')
import examples.mujoco.reproduction.td3.core_cuda as core
...
class PseudoEncoderSpikeRegular(torch.autograd.Function):
    ...
    def forward(ctx, input):
        # print("forward",input.gt(ENCODER_REGULAR_VTH).float())
...
class PopSpikeEncoderRegularSpike(nn.Module):
    ...
    def forward(self, obs, batch_size=100):
        """
        ...
        #print("obs",obs)
        #print("pop_spikes",pop_spikes)
        return pop_spikes


class PopSpikeDecoder(nn.Module):
    """ Population Coding Spike Decoder """
    def __init__(self, act_dim, pop_dim, output_activation=nn.ReLU):
        """
...
class SpikeMLP(nn.Module):
    ...
    def forward(self, in_pop_spikes, batch_size=100):
        """
...
class PopSpikeActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, en_pop_dim, de_pop_dim, hidden_sizes,
        ...
        self.decoder = PopSpikeDecoder(act_dim, de_pop_dim, output_activation=nn.Tanh)
    ...
    def forward(self, obs, batch_size=100):
        ...
        action = self.decoder(out_pop_activity)
        #print(action.size())
        base_distribution = torch.distributions.Independent(Delta(loc=action), 1)
        #return torch.distributions.Independent(Delta(loc=action), 1)
