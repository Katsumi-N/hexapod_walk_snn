...
_HEIGHTFIELD_ID = 0
_TERRAIN_SMOOTHNESS = 0.3  # 0.0: maximally bumpy; 1.0: completely smooth.
...
def make_model(floor_size=None, terrain=False, rangefinders=False,
               walls_and_ball=False, target=False):
  """Returns the model XML string."""
  xml_string = common.read_model('hexapod.xml')
  # xml_string = common.read_model('hexapod_r1.xml')
...
def walk(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  ...
  task = Move_gym(desired_speed=_WALK_SPEED, random=random)
...
# 特定のポイントに向かうタスク
@SUITE.add()
def walk_point(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns the Fetch task."""
  xml_string = make_model(walls_and_ball=True, target=True)
  physics = Physics.from_xml_string(xml_string, common.ASSETS)
  task = Move_point(random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(physics, task, time_limit=time_limit,
                             control_timestep=_CONTROL_TIMESTEP,
                             **environment_kwargs)
...
class Physics(mujoco.Physics):
  ...
  def force_torque(self):
    ...
    obs_torque = ["torque_toe_middle_left", "torque_toe_middle_right", "torque_toe_front_left",
                  "torque_toe_front_right", "torque_toe_back_right","torque_toe_back_left"]
    # force_torque_sensors = self._get_sensor_names(enums.mjtSensor.mjSENS_FORCE,
    #                                               enums.mjtSensor.mjSENS_TORQUE)
    force_torque_sensors = self._get_sensor_names(enums.mjtSensor.mjSENS_FORCE)
    return np.arcsinh(self.named.data.sensordata[force_torque_sensors + obs_torque])
  ...
  def get_energy(self):
    torque_sensors = ['torque_hip_front_left', 'torque_hip_front_left','torque_knee_front_left', 'torque_ankle_front_left',
                      'torque_hip_front_right','torque_hip_front_right','torque_knee_front_right', 'torque_ankle_front_right',
                      'torque_hip_middle_left','torque_hip_middle_left','torque_knee_middle_left', 'torque_ankle_middle_left',
                      'torque_hip_middle_right','torque_hip_middle_right','torque_knee_middle_right', 'torque_ankle_middle_right',
                      'torque_hip_back_left','torque_hip_back_left','torque_knee_back_left', 'torque_ankle_back_left',
                      'torque_hip_back_right','torque_hip_back_right','torque_knee_back_right', 'torque_ankle_back_right']
    joint_name = ['yaw_front_left', 'pitch_front_left', 'knee_front_left', 'ankle_front_left',
                  'yaw_front_right', 'pitch_front_right', 'knee_front_right', 'ankle_front_right',
                  'yaw_middle_left', 'pitch_middle_left', 'knee_middle_left', 'ankle_middle_left',
                  'yaw_middle_right', 'pitch_middle_right', 'knee_middle_right', 'ankle_middle_right',
                  'yaw_back_left', 'pitch_back_left', 'knee_back_left', 'ankle_back_left',
                  'yaw_back_right', 'pitch_back_right', 'knee_back_right', 'ankle_back_right']
    angular_velocity = self.named.data.qvel[joint_name]
    energy = 0
    for i in range(len(torque_sensors)):
      t = self.named.data.sensordata[torque_sensors[i]]
      vel = angular_velocity[i]
      name = torque_sensors[i].split('_')[1]
      if name == "ankle":
        torque = t[2]
      elif name == "knee":
        torque = t[1]
        ...
        assert name == "hip"
        joint = joint_name[i].split('_')[0]
        if joint == "yaw":
          torque = t[2]
        else:
          assert joint == "pitch"
          torque = t[1]
        # print(i+1, "torque:",abs(torque), "vel",abs(vel))
        # print(i+1, "torque:",np.arcsinh(abs(torque)), "vel",np.arcsinh(abs(vel)))
      energy += 0.001 * abs(torque) * abs(vel)
    return energy

  def center_of_mass(self):
    """Returns position of the center-of-mass."""
    position = self.named.data.subtree_com['torso'].copy()
    vel = self.named.data.sensordata['torso_subtreelinvel'].copy()
    return position, vel
...
class Move_gym(base.Task):
  ...
  def get_reward(self, physics):
    ...
    #print("vel",physics.torso_velocity())
    if physics.torso_upright() > -0.8:
      healthy = 1
    else:
      healthy = 0
    reward = x_velocity + healthy
    #reward = x_velocity
    return reward
...
class Move_point(base.Task):
  ...
  def get_reward(self, physics):
    ...
    # ターゲットとエージェントの距離
    distance = np.linalg.norm(physics.named.data.site_xpos['target'] - physics.named.data.xpos['torso'])
    ...
    if physics.torso_upright() > -0.8:
      ...
      healthy = 0
    reward = x_velocity + healthy + distance_reward
    return reward